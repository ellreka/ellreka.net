...
人間が Google Drive に画像をアップロードしたら自動で Spreadsheet に書き込まれることを目指しました。

1. 画像を Google Drive にアップロードする (人間)
2. 定期的に Google Drive に追加された画像を取得する (n8n)
3. 画像を OCR で JSON 化する (Mastra)
...
特に種目名の抽出がかなりばらつきがあったのでこちらで事前に候補を与えそれに近しいものを選ばせるようにしました。
...
Mastra では workflow を実行するための API がいくつか用意されていますが、非同期で結果を受け取るために `/startAsync` を使います。
